...
document.getElementById("cash").addEventListener("change", runTheRegister);
document.getElementById("bill").addEventListener("change", runTheRegister);
...
function runTheRegister() {
   ...
   try {
    ...
    if (changeValue < 0) throw "Cash amount doesn't cover the bill" // throw an exception if the change is less than zero (bill is higher than cash amount)
    ...
    document.getElementById("warning").innerHTML = error // display the error message in the warning div
   }
}
